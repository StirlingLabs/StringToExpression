...
public class ParserErrorTests {
  ...
  public ParserErrorTests() {
    ...
    BaseGrammarDefinition listDelimiter;
    Language = new(
      ...
      errorFn = new FunctionCallDefinition(
        ...
        @"error\(",
        new[] { typeof(double), typeof(double) },
        _ => throw new NotImplementedException("I am a function error")),
      openBracket = new(
        "OPENBRACKET",
        @"\("),
      ...
      new OperandDefinition(
        "POOP",
        @"💩",
        _ => throw new NotImplementedException("I am an operand error")),
      new OperandDefinition(
        "STRING",
        @"'.*?'",
        x => Expression.Constant(x.ToString())),
